...
for i in range(args.f,args.t ):
	...
	train_gen = hist_data_generator(files = t,seed = seed, batch_size = batch_size,contiguous=True,lock=mutex)
	val_gen = hist_data_generator(files = v,seed = seed, batch_size = batch_size,contiguous=True,lock=mutex)
	...
	filepath = 'models/fold{}.{}l.{}h.{}-last.LSTM.weights.best.hdf5'.format(i,layers,hidden_units,args.last)
